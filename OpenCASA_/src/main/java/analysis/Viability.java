...
public class Viability implements Measurements,ChangeListener, MouseListener{
	...
	float	minSize = (float) (Params.minSize*pixelsPerUm);//minimum sperm size (um^2)
	float	maxSize = (float) (Params.maxSize*pixelsPerUm);//maximum sperm size (um^2)	
...
/******************************************************/
	...
	public void checkSelection(int x, int y){
		...
		switch(activeImage){
			case 1: thespermatozoa = aliveSpermatozoa;
					break;
			case 2: thespermatozoa = deadsSpermatozoa;
					break;						
		}
	...
	public void createGUI() {
		...
		btnAlive.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) { 
				...
				if(aliveImpOrig!=null){//Usefull when the user cancel before load an image
					activeImage = 1;
		...
		btnDeads.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) { 
				...
				if(deadsImpOrig!=null){//Usefull when the user cancel before load an image
					activeImage = 2;
		...
		aliveOtsuButton.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) {
				...
				aliveThreshold = -1.0;
				if((aliveImpOrig!=null)&&(activeImage==1)){//Usefull when the user cancel before load an image
					aliveImpDraw = aliveImpOrig.duplicate();
					imp.setProcessor(aliveImpDraw.getProcessor());
					imp.setTitle("Alive");
					imp.show();
		...
		deadsOtsuButton.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) {
				...
				deadsThreshold = -1.0;
				if((deadsImpOrig!=null)&&(activeImage==2)){//Usefull when the user cancel before load an image
					deadsImpDraw = deadsImpOrig.duplicate();
					imp.setProcessor(deadsImpDraw.getProcessor());
					imp.setTitle("Deads");
					imp.show();
    ...
    private void doMouseRefresh() {
		if(!isThresholding){
			...
			Thread t1 = new Thread(new Runnable() {
				public void run() {
					switch(activeImage){
						case 1: aliveImpDraw = aliveImpOrig.duplicate();
								imp.setProcessor(aliveImpDraw.getProcessor());
								setAliveImage(false);
								break;
						case 2: deadsImpDraw = deadsImpOrig.duplicate();
								imp.setProcessor(deadsImpDraw.getProcessor());
								setDeadsImage(false);
								break;						
					}
	...
	public void drawBoundaries(ImagePlus imp,List thespermatozoa){
		...
		switch(activeImage){
			case 1: ip.setColor(Color.green);
					break;
...
public boolean isOutsideBorder(Spermatozoon part){
	...
	/*public int setup(String arg, ImagePlus imp) {
		if(imp==null)
			return DOES_RGB;
		this.aliveImpOrig = imp.duplicate();
		this.deadsImpOrig = imp.duplicate();
		this.imp=imp;
