...
public class AVI_Reader extends VirtualStack implements PlugIn {
  ...
  class raInputStream extends InputStream {
    ...
    boolean fixMJPG; // whether to use an ugly hack to convert MJPG frames to
                     // JPEG
    ...
    private void addHuffmanTables() {
      ...
      do {
        ...
        if (code == 0xffc4) // Huffman table found, nothing to do
          return;
        else if (code == 0xffda || code == 0xffd9) { // start of image data
                                                     // or end of image?
  ...
  private final static long SIZE_MASK = 0xffffffffL; // for conversion of sizes
   ...
   */
  final private static int BUFFERSIZE = 4096; // should be large enough to hold
                                              // the full JFIF header
  // up to beginning of the image data and the Huffman tables
  final private static byte[] HUFFMAN_TABLES = new byte[] { // the 'DHT'
                                                            // segment
      (byte) 0xFF, (byte) 0xC4, 0x01, (byte) 0xA2, // these 4 bytes are tag &
                                                   // length; data follow
      ...
      (byte) 0x81, (byte) 0x91, (byte) 0xA1, 0x08, 0x23, 0x42, (byte) 0xB1, (byte) 0xC1, 0x15, 0x52, (byte) 0xD1,
      (byte) 0xF0, 0x24, 0x33, 0x62, 0x72, (byte) 0x82, 0x09, 0x0A, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x25, 0x26, 0x27,
      ...
      0x77, 0x78, 0x79, 0x7A, (byte) 0x83, (byte) 0x84, (byte) 0x85, (byte) 0x86, (byte) 0x87, (byte) 0x88, (byte) 0x89,
      ...
      (byte) 0xF9, (byte) 0xFA, 0x11, 0x00, 0x02, 0x01, 0x02, 0x04, 0x04, 0x03, 0x04, 0x07, 0x05, 0x04, 0x04, 0x00,
      0x01, 0x02, 0x77, 0x00, 0x01, 0x02, 0x03, 0x11, 0x04, 0x05, 0x21, 0x31, 0x06, 0x12, 0x41, 0x51, 0x07, 0x61, 0x71,
      0x13, 0x22, 0x32, (byte) 0x81, 0x08, 0x14, 0x42, (byte) 0x91, (byte) 0xA1, (byte) 0xB1, (byte) 0xC1, 0x09, 0x23,
      0x33, 0x52, (byte) 0xF0, 0x15, 0x62, 0x72, (byte) 0xD1, 0x0A, 0x16, 0x24, 0x34, (byte) 0xE1, 0x25, (byte) 0xF1,
      0x17, 0x18, 0x19, 0x1A, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x43, 0x44, 0x45, 0x46,
      0x47, 0x48, 0x49, 0x4A, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69,
      0x6A, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, (byte) 0x82, (byte) 0x83, (byte) 0x84, (byte) 0x85,
      ...
      (byte) 0xE8, (byte) 0xE9, (byte) 0xEA, (byte) 0xF2, (byte) 0xF3, (byte) 0xF4, (byte) 0xF5, (byte) 0xF6,
  ...
  private boolean convertToGray = false; // whether to convert color video to
                                         // grayscale
  ...
  private int type0xdb, type0xdc; // video stream chunks must have one of these
                                  // two types (e.g. '00db' for straem 0)
  ...
  private boolean indexForCountingOnly;// don't read the index, only count int
                                       // totalFramesFromIndex how many entries
  ...
  private short biPlanes; // no. of color planes: for the formats decoded; here
                          // always 1
  ...
  /**
   * Find the next position of fourcc or LIST fourcc and read contents. Returns
   * next position If not found, throws exception or returns -1
   */
   ...
   * returns the first position inside the fourcc chunk and puts the file
  ...
  private long findFourccAndSkip(int fourcc, boolean isList, long endPosition) throws IOException {
    while (true) {
      ...
      long nextPos = chunkPos + size; // note that 'size' of a list includes
                                      // the 'type' that follows now
      if (isList && type == FOURCC_LIST)
        type = readInt();
      if (verbose)
   ...
   * it is one) where 1<=n<=nslices. Returns null if no virtual stack or no
   * slices.
   */
   ...
   * @param lastFrame
   *          Number of last frame to read or 0 for reading all, -1 for all but
   *          last...
   * @param isVirtual
   ...
   *         found
   */
  public ImageStack makeStack(String path, int firstFrame, int lastFrame, boolean isVirtual, boolean convertToGray,
    ...
    try {
      ...
      stack.trim();
      e.printStackTrace();
      IJ.showMessage("AVI Reader",
          "Out of memory.  " + stack.getSize() + " of " + dwTotalFrames + " frames will be opened.");
    } catch (Exception e) {
      exceptionMessage = exceptionMessage(e);
    } finally {
      try {
  ...
  private void readAVI(String path) throws Exception, IOException {
    ...
    startTime += System.currentTimeMillis();// taking previously elapsed time
                                            // into account
    ...
    if (isVirtual || firstFrame > 1) { // avoid scanning frame-by-frame where
                                       // we only need the positions
      frameInfos = new Vector(100); // holds frame positions, sizes and time
                                    // since start
      long nextPosition = -1;
    ...
    // IJ.log("at 0x"+Long.toHexString(pos)+"
  ...
  /**
   * Read 'AVI 2'-type main index 'indx' or an 'ix00' index to frames (only the
   * types AVI_INDEX_OF_INDEXES and AVI_INDEX_OF_CHUNKS are supported)
   */
  private void readAvi2Index(long endPosition) throws Exception, IOException {
    ...
    if (bIndexType == AVI_INDEX_OF_INDEXES) { // 'indx' points to other
      ...
      if (wLongsPerEntry != 2)
        return; // badly formed index, ignore it
      if (dwChunkId != type0xdb && dwChunkId != type0xdc) { // not the stream
                                                            // we search for?
                                                            // (should not
                                                            // happen)
        if (verbose)
          IJ.log("INDEX ERROR: SKIPPED ix00, wrong stream number or type, should be " + fourccString(type0xdb) + " or "
              + fourccString(type0xdc));
        return;
      }
      if (indexForCountingOnly) { // only count number of entries, don't put
                                  // into table
      ...
      for (int i = 0; i < nEntriesInUse; i++) {
        ...
        if (frameNumber >= firstFrame) {
          ...
          // if (verbose)
          // IJ.log("movie data "+frameNumber+"
          // '"+fourccString(dwChunkId)+"'
  ...
  void readBitMapInfo(long endPosition) throws Exception, IOException {
    ...
    case NO_COMPRESSION_RAW:
      ...
      dataTopDown = biHeight < 0; // RGB mode is usually bottom-up, negative
                                  // height signals top-down
    ...
    if (allowedBitCount != 0 && (bitCountTest & allowedBitCount) == 0)
      throw new Exception(
          "Unsupported: " + biBitCount + " bits/pixel for compression '" + fourccString(biCompression) + "'");
  ...
  private boolean readContents(int fourcc, long endPosition) throws Exception, IOException {
    ...
    case FOURCC_strl:
      ...
      indexForCountingOnly = true; // try reading indx for counting number of
                                   // entries
  ...
  void readMovieData(long endPosition) throws Exception, IOException {
    ...
    if (verbose)
      IJ.log("Searching for stream " + streamNumber + ": '" + fourccString(type0xdb) + "' or '" + fourccString(type0xdc)
          + "' chunks");
    if (isVirtual) {
      if (frameInfos == null) // we might have it already from reading the
                              // first chunk
        frameInfos = new Vector(100); // holds frame positions in file (for
                                      // non-constant frame sizes, should
                                      // hold long[] with pos and size)
    } else if (stack == null)
  ...
  private void readOldFrameIndex(long endPosition) throws Exception, IOException {
    int offset = -1; // difference between absolute frame address and address
                     // given in idx1
    int[] offsetsToTry = new int[] { 0, (int) moviPosition }; // dwOffset may
                                                              // be w.r.t.
                                                              // file start or
                                                              // w.r.t. 'movi'
                                                              // list.
    while (true) {
      ...
      if ((dwChunkId == type0xdb || dwChunkId == type0xdc) && dwSize > 0) {
        if (offset < 0) { // find out what the offset refers to
          ...
          for (int i = 0; i < offsetsToTry.length; i++) {
            ...
            int chunkIdAtPos = readInt(); // see whether this offset
                                          // points to the desired chunk
            // IJ.log("read@=0x"+Long.toHexString(pos)+":
        ...
        if (frameNumber >= firstFrame) {
          ...
          // if (verbose)
          // IJ.log("idx1 movie data '"+fourccString(dwChunkId)+"'
          // "+posSizeString(framepos,dwSize)+timeString());
        }
        frameNumber++;
        if (frameNumber > lastFrameToRead)
          break;
  ...
  void readStreamHeader() throws Exception, IOException { // 'strh'
    ...
    type0xdb = FOURCC_00db + (streamNumber << 8); // '01db' for stream 1, etc.
                                                  // (inverse byte order!)
    type0xdc = FOURCC_00dc + (streamNumber << 8); // '01dc' for stream 1, etc.
  }

  /**
   * Read type of next chunk that is not JUNK. Returns type (or 0 if no non-JUNK
   * chunk until endPosition)
   */
  ...
  /**
   * The plugin is invoked by ImageJ using this method. String 'arg' may be used
   * to select the path.
   */
  public void run(String arg) {
    ...
    try {
      ImageStack stack = makeStack(path, firstFrame, lastFrame, isVirtual, convertToGray, flipVertical); // read
                                                                                                         // data
    } catch (Exception e) {
      error(exceptionMessage(e));
      return;
  ...
  void unpack(byte[] rawData, int rawOffset, int[] pixels, int intOffset, int w) {
    ...
    case YUY2_COMPRESSION:
      for (int i = 0; i < w / 2; i++) {
        ...
        int u = rawData[k++] ^ 0xffffff80; // converts byte range 0...ff to
                                           // -128 ... 127
  ...
  void unpackGray(byte[] rawData, int rawOffset, byte[] pixels, int byteOffset, int w) {
    ...
    if (dataCompression == 0) {
      ...
      for (int i = 0; i < w; i++) {
        pixels[j++] = rawData[k]; // Non-standard: no scaling from 16-235 to
                                  // 0-255 here
        k += step;
      }
    }
