...
public class AVI_Reader extends VirtualStack implements PlugIn {
   ...
   final private static byte[] HUFFMAN_TABLES = new byte[] { // the 'DHT'
         ...
         0x07, 0x22, 0x71, 0x14, 0x32, (byte) 0x81, (byte) 0x91, (byte) 0xA1, 0x08, 0x23, 0x42, (byte) 0xB1,
         (byte) 0xC1, 0x15, 0x52, (byte) 0xD1, (byte) 0xF0, 0x24, 0x33, 0x62, 0x72, (byte) 0x82, 0x09, 0x0A, 0x16, 0x17,
         ...
         0x67, 0x68, 0x69, 0x6A, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, (byte) 0x83, (byte) 0x84, (byte) 0x85,
         ...
         (byte) 0xF5, (byte) 0xF6, (byte) 0xF7, (byte) 0xF8, (byte) 0xF9, (byte) 0xFA, 0x11, 0x00, 0x02, 0x01, 0x02,
         0x04, 0x04, 0x03, 0x04, 0x07, 0x05, 0x04, 0x04, 0x00, 0x01, 0x02, 0x77, 0x00, 0x01, 0x02, 0x03, 0x11, 0x04,
         0x05, 0x21, 0x31, 0x06, 0x12, 0x41, 0x51, 0x07, 0x61, 0x71, 0x13, 0x22, 0x32, (byte) 0x81, 0x08, 0x14, 0x42,
         (byte) 0x91, (byte) 0xA1, (byte) 0xB1, (byte) 0xC1, 0x09, 0x23, 0x33, 0x52, (byte) 0xF0, 0x15, 0x62, 0x72,
         (byte) 0xD1, 0x0A, 0x16, 0x24, 0x34, (byte) 0xE1, 0x25, (byte) 0xF1, 0x17, 0x18, 0x19, 0x1A, 0x26, 0x27, 0x28,
         0x29, 0x2A, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x53, 0x54,
         0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x73, 0x74, 0x75, 0x76,
         0x77, 0x78, 0x79, 0x7A, (byte) 0x82, (byte) 0x83, (byte) 0x84, (byte) 0x85, (byte) 0x86, (byte) 0x87,
    ...
    * (if it is one) where 1<=n<=nslices. Returns null if no virtual stack or no
    * slices.
    */
      ...
      if (bIndexType == AVI_INDEX_OF_INDEXES) { // 'indx' points to other
         ...
         for (int i = 0; i < nEntriesInUse; i++) {
            ...
            if (verbose) {
               ...
               IJ.log("   index data '" + fourccString(dwChunkId) + "' " + posSizeString(qwOffset, dwSize)
                     + timeString());
            }
         ...
         if (dwChunkId != type0xdb && dwChunkId != type0xdc) { // not the stream
                                                               ...
                                                               // happen)
            if (verbose)
               IJ.log("INDEX ERROR: SKIPPED ix00, wrong stream number or type, should be " + fourccString(type0xdb)
                     + " or " + fourccString(type0xdc));
            return;
         }
   ...
   void readMovieData(long endPosition) throws Exception, IOException {
      ...
      if (verbose)
         IJ.log("Searching for stream " + streamNumber + ": '" + fourccString(type0xdb) + "' or '"
